...
class KasaSistemi(ctk.CTk):
    def __init__(self):
        ...
        self.menu_et = menuyu_guncelle(baslangic_menu_et)
        self.menu_tavuk = menuyu_guncelle(baslangic_menu_tavuk)
        self.menu_kampanya = menuyu_guncelle(baslangic_menu_kampanya)
        ...
        tepe_kutu.pack(fill="x", padx=10, pady=10)
        self.baslik = ctk.CTkLabel(tepe_kutu, text="SARAÇOĞLU", font=("Arial", 36, "bold"), text_color="#FF9800")
        ...
        self.yazici_btn.pack(side="left", padx=5)
        self.kapat_btn = ctk.CTkButton(btn_kutu, text="🛑 Çıkış", font=("Arial", 18, "bold"), fg_color="#1E1E1E", hover_color="#b71c1c", command=self.sistemi_tamamen_kapat, width=90, height=40)
        self.kapat_btn.pack(side="left", padx=5)
        ...
        self.icerik_frame.bind("<Configure>", self.icerik_resize_oldu)
    ...
    def telefondan_siparis_geldi(self, json_veri):
        ...
        if len(json_veri.get("kalemler", [])) > 0:
            ...
            self.aktif_siparisler.append({"musteri_adi": musteri, "saat": saat, "kalemler": temiz, "toplam_tutar": int(json_veri["toplam_tutar"]), "durum": "Bekliyor"})
    ...
    def menuyu_ciz(self, menu_listesi):
        ...
        for urun in menu_listesi:
            ...
            self.icerik_frame.grid_rowconfigure(satir, weight=1)
            sutun += 1
    ...
    def siparis_penceresi_ac(self, urun):
        ...
        w, h = 950, 750; x = (self.winfo_screenwidth() - w) // 2; y = (self.winfo_screenheight() - h) // 2
        ...
        govde = ctk.CTkScrollableFrame(popup, fg_color="transparent"); govde.pack(fill="both", expand=True, padx=10)
    ...
    def masayi_kaydet(self):
        ...
        self.aktif_siparisler.append({"musteri_adi": isim, "saat": datetime.datetime.now().strftime("%H:%M"), "kalemler": self.sepet.copy(), "toplam_tutar": sum(k['fiyat'] for k in self.sepet), "durum": "Bekliyor"})
        self.masalari_diske_kaydet_ve_yay()
        self.sepeti_temizle()
        if self.secili_sekme_index == 0: self.acik_masalari_ciz()
    ...
    def fiyat_penceresi_ac(self, urun):
        ...
        kutu_frame = ctk.CTkScrollableFrame(popup, fg_color="transparent"); kutu_frame.pack(fill="both", expand=True, padx=15)
        girdi_kutulari = {}
